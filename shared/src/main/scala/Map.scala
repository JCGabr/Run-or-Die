...
class MapGame(
  ...
  @tailrec
  private final def weightedRandomAuxiliar[T](
      random: Float,
      map_probability: Seq[(T, Float)]
  ): T = {
  ...
  def adjustWeightByY(
      map_probability: Map[String, Float],
      y: Int
  ): Map[String, Float] = {
    ...
    val platformFamilies = Set(
      ...
      "platform_diag1",
      "platform_diag2",
      "platform_cube_top_right",
      "platform_cube_top_left",
      "platform_cube_bottom_right",
      "platform_cube_bottom_left"
    )
        ...
        val columnState =
          (0 to max_y).foldLeft(
            ...
            ColumnState(
              ...
              checkpointPlaced =
                columnState.checkpointPlaced || isCheckpoint(segment)
            )
          }
