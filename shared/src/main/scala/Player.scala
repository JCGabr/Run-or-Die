...
case class Player(
  ...
  def findHazard(
    ...
    cellsOverlapping(x, y, width, height, cellSize, cells).find(c =>
      c == "S" || c == "C"
    )
  ...
  def resolveCollisions(
    ...
    val touchesFloor = vel.y >= 0 && isBlocked(
      coords.x,
      proposed.y + 1f,
      width,
      height,
    ...
    val groundedY =
      if (touchesFloor) (snapRowFoot * cellSize - height).toFloat
      else proposed.y
    ...
    val touchesCeiling = vel.y < 0 && isBlocked(
      coords.x,
      proposed.y,
      width,
      height,
    ...
    val finalY =
      if (touchesCeiling) ((snapRowHead + 1) * cellSize).toFloat else groundedY
    ...
    val touchesWall = vel.x != 0 && isBlocked(
      proposed.x,
      finalY,
      width,
      height,
    ...
    findHazard(finalX, finalY, width, height, cellSize, cells) match {
      case Some("S") =>
        resolved.copy(
          coords = last_checkpoint,
          velocity = Vector2(0f, 0f),
          current_time = current_time - 1
        )
      case Some("C") if finalX > last_checkpoint.x + cellSize * 3 =>
        resolved.copy(
          last_checkpoint = Vector2(finalX, finalY),
          current_time = max_time,
          claimed_checkpoint = true
        )
      case _ =>
        resolved
    }
